...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal,
  ...
  for (int i = 0; i < lights.size(); ++i) {
    ...
    /* If the angle is more than +/- 90 degrees, the dot product is negative (n
    and l are unit vectors) so reflection does not "illuminate" this point more
  */
    if (phi > 0) {
      // Add specular illumination, diffuse and multiply by the light intensity
      glm::vec3 r = ((2.0f * normal) * phi) - l;
      color += (material.specular *
                    pow(glm::dot(r, view_direction), material.shininess) +
                material.diffuse * phi) *
               lights[i]->color;
    }
  }
...
glm::vec3 trace_ray(Ray ray) {
  ...
  // If the ray didn't hit anything, we save the color 0,0,0 (black) as our
  // background color
  if (closest_hit.hit) {
    /* ------------------Exercise 3--------------------
...
void sceneDefinition() {
  ...
  // objects.push_back(new Sphere(2.0, glm::vec3(0, 0, 9), glm::vec3(0.6, 0.9,
  // 0.6)));
  ...
  ------------------------------------------------- */
  // objects.push_back(new Sphere(1.0, glm::vec3(1.0, -2.0, 8.0), glm::vec3(0.6,
  // 0.6, 0.9))); Q: Why name it like this?
